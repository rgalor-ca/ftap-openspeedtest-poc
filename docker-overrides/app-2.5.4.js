    ...
    function OpenSpeedtest() {
      ...
      var Engine = setInterval(function() {
        ...
        if (Status === "Error") {
          ...
          circleSVG.style.cursor = "default";
          circleSVG.onclick = null;
        }
        if (Status === "SendR") {
          Show.showStatus("All done");
          var restartOpenSpeedTest = function() {
            var url = new URL(window.location.href);
            url.searchParams.set("Run", "1");
            url.searchParams.set("pocRefresh", Date.now().toString());
            window.location.href = url.toString();
          };
          Show.OpenSpeedtest.el.style.cursor = "pointer";
          Show.OpenSpeedtest.el.onclick = function(event) {
            if (Show.oDoLiveStatus.el.textContent === "All done") {
              event.preventDefault();
              restartOpenSpeedTest();
            }
          };
          Show.oDoLiveStatus.el.style.cursor = "pointer";
          Show.oDoLiveStatus.el.onclick = function() {
            restartOpenSpeedTest();
          };
          Show.oDoLiveSpeed.el.textContent = ost;
          var circleSVG = document.getElementById("oDoLiveSpeed");
          circleSVG.style.cursor = "pointer";
          circleSVG.onclick = function() {
            restartOpenSpeedTest();
          };
